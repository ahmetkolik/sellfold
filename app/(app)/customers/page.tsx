...
import { Download, Crown, Mail, ShoppingBag, Wallet } from "lucide-react";
...
export default function CustomersPage() {
  ...
  const shown = vipOnly ? customers.filter((c) => c.vip) : customers;
  const totalRevenue = customers.reduce((a, c) => a + c.spent, 0);
  const repeat = customers.filter((c) => c.orders > 1).length;

  const totals = [
    { label: m.total, value: formatNumber(customers.length) },
    { label: m.revenue, value: formatMoney(totalRevenue) },
    { label: m.repeat, value: `${repeat} / ${customers.length}` },
  ];
