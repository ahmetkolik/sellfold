      ...
      <Link href={`/p/${product.id}`} className="block">
        <div className="relative">
          <div
            ...
            style={!product.category_image_url ? { backgroundImage: `linear-gradient(140deg, oklch(94% 0.06 ${product.hue}) 0%, oklch(86% 0.13 ${product.hue}) 100%)` } : undefined}
          >
            {product.category_image_url
              ? <img src={product.category_image_url} alt={product.title} className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-105" />
              : <span className="flex h-full w-full items-center justify-center text-4xl drop-shadow-sm">{product.emoji}</span>
            }
          </div>
...
export default function StorePage() {
  ...
  useEffect(() => {
    ...
    supabase
      .from("products")
      .select("id, title, type, price, hue, emoji, category_image_url")
      .eq("live", true)
